<script lang="ts">
  ...
  function isValidNumber(board: number[][], row: number, col: number, num: number): boolean {
    ...
    if (row + col === 5) {
      for (let i = 0; i < 6; i++) {
        if (i !== row && board[i][5 - i] === num) return false;
      }
    }
  ...
  function validateDiagonals(board: number[][]): boolean {
    ...
    if (mainDiagSet.size !== 6 || !Array.from(mainDiagSet).every(n => n >= 1 && n <= 6)) {
      return false;
    }
    ...
    const antiDiag = [];
    for (let i = 0; i < 6; i++) {
      antiDiag.push(board[i][5 - i]);
    }
    const antiDiagSet = new Set(antiDiag);
    if (antiDiagSet.size !== 6 || !Array.from(antiDiagSet).every(n => n >= 1 && n <= 6)) {
      return false;
    }
  ...
  function getCellClass(row: number, col: number): string {
    ...
    const isAntiDiagonal = row + col === 5;
...
<svelte:head>
  <title>대각선 스도쿠 정답 생성기</title>
  <meta name="description" content="무작위 6x6 대각선 스도쿠 정답을 생성합니다." />
  <!-- 모바일화면 확대 금지 -->
...
<div class="min-h-screen bg-gradient-to-br from-green-50 to-blue-100 p-4">
  <div class="max-w-4xl mx-auto">
    ...
    <div class="text-center mb-8">
      <h1 class="text-4xl font-bold text-gray-800 mb-2">대각선 스도쿠 정답 생성기</h1>
      <p class="text-gray-600">완전히 채워진 무작위 6x6 대각선 스도쿠 정답을 생성합니다</p>
    </div>
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg p-6 shadow-lg">
          ...
          <ul class="text-sm text-gray-600 space-y-2">
            ...
            <li>• <strong>대각선 제약 조건</strong>: 두 대각선에도 1~6 숫자가 고유하게 배치</li>
        ...
        <!-- 스도쿠 규칙 -->
        <div class="bg-white rounded-lg p-6 shadow-lg">
          <h2 class="text-xl font-bold mb-4">6x6 대각선 스도쿠 규칙</h2>
          <ul class="text-sm text-gray-600 space-y-2">
            <li>• 각 행에 1~6 숫자가 하나씩만 포함</li>
            <li>• 각 열에 1~6 숫자가 하나씩만 포함</li>
            <li>• 각 2×3 박스에 1~6 숫자가 하나씩만 포함</li>
            <li>• <span class="text-blue-600 font-semibold">메인 대각선</span>에 1~6 숫자가 하나씩만 포함</li>
            <li>• <span class="text-green-600 font-semibold">반대 대각선</span>에 1~6 숫자가 하나씩만 포함</li>
            <li>• 총 36개 셀, 6개 박스, 2개 대각선으로 구성</li>
          </ul>
